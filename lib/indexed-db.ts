...
class PilotLogbookDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(9).stores({
      ...
      airports: "icao, iata, name, id, isFavorite", // No sync fields needed
...
export async function silentDeleteFlight(id: string): Promise<boolean> {
  ...
  if (!flight) {
    ...
    const byMongoPattern = await db.flights
      .filter((f) => f.id === id || f.mongoId === id)
      .first();
    if (byMongoPattern) {
...
export async function silentDeleteAircraft(id: string): Promise<boolean> {
  const aircraft = await db.aircraft.get(id);
  if (!aircraft) {
    const byMongoPattern = await db.aircraft
      .filter((a) => a.id === id || a.mongoId === id)
      .first();
    if (byMongoPattern) {
...
// Update the helper to use the new "tz" property
...
export async function bulkLoadAirports(
  rawData: Record<string, any>
): Promise<void> {
  const DATA_VERSION = "2025.10.27-min";
  // 1. Get current favorites first
  const existingFavorites = await db.airports
    .where("isFavorite")
    .equals(1)
    .toArray();
  ...
  const airports: Airport[] = Object.values(rawData)
    .map((airport: any, index: number) => ({
      ...
      tz: airport.tz || "UTC",
      // 2. Restore favorite status if it existed
...
/**
 * Toggles the favorite status of an airport.
 * Uses the ICAO as the key since it's the primary key in your schema.
 */
...
/**
 * Gets only the airports marked as favorite
 */
export async function getFavoriteAirports(): Promise<Airport[]> {
  return db.airports.where("isFavorite").equals(1).toArray(); // Dexie treats true as 1 in indexes
}
...
export async function silentDeletePersonnel(id: string): Promise<boolean> {
  const person = await db.personnel.get(id);
  if (!person) {
    const byMongoPattern = await db.personnel
      .filter((p) => p.id === id || p.mongoId === id)
      .first();
    if (byMongoPattern) {
...
export async function clearAllUserData(): Promise<void> {
  await db.transaction(
    "rw",
    [db.flights, db.aircraft, db.personnel, db.syncQueue, db.syncMeta],
    async () => {
