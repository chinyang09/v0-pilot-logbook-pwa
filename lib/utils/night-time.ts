...
export function interpolateGreatCircle(
  ...
  if (d < 0.0001) return [lat1 + fraction * (lat2 - lat1), lon1 + fraction * (lon2 - lon1)]
...
function calculateSolarElevation(date: Date, lat: number, lon: number): number {
  ...
  const epsilon = 23.439291 - jc * (0.0130042 + jc * (0.00000016 - jc * 0.000000504))
  ...
  const trueSolarTime =
    date.getUTCHours() * 60 + date.getUTCMinutes() + date.getUTCSeconds() / 60 + timeOffset
