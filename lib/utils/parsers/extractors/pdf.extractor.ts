/**
 * PDF extractor — turns a PDF file into normalized rows.
 *
 ...
 *
 * Tokens are snapped to the header row's X-anchors, so an empty cell in the
 * PDF layout becomes an empty cell — preserving the night-takeoff vs
 * night-landing distinction the logbook parser depends on. The output is the
 * same shape the CSV extractor produces, so both converge on the shared
 * parser layer with no field parsing in either extractor.
 */
...
type GetDocumentProxy = (data: Uint8Array) => Promise<PdfDocumentProxy>;
...
/**
 * Snap a row's tokens to the header column anchors, returning one cell per
 * anchor (empty string where the layout cell was blank), plus any tokens
 * that fall before/after the anchor band. Rows seen before the header has
 * been located (no anchors yet) are gap-joined and split on the inserted
 * commas.
 */
function rowToCells(row: Row, anchors: number[]): string[] {
  if (anchors.length === 0) {
    const joined = gapJoinRow(row);
...
export async function extractPdfRows(file: File): Promise<{
  rows: NormalizedRow[];
  rawText: string;
}> {
  ...
  try {
    ...
    for (let p = 1; p <= pdf.numPages; p++) {
      ...
      const pageRows = bucketRows(items);
      const pageAnchors = findHeaderColumns(pageRows);
    ...
    return { rows, rawText: rows.map((r) => r.raw).join("\n") };
  } catch (err) {
