...
class SyncService {
  ...
  private async getAuthHeaders(): Promise<HeadersInit> {
    ...
    return {
      ...
      // removed"x-user-id": session.userId,
    };
  }
  ...
  async pullFromServer(): Promise<{ count: number }> {
    ...
    const lastSyncTime = rawLastSyncTime ? Number(rawLastSyncTime) : 0;
    console.log(
      "[v0] Last sync time:",
      lastSyncTime,
          ...
          if (response.ok) {
            ...
            for (const record of records) {
              try {
                ...
                console.error(
                  `[v0] Error upserting ${collection} record:`,
                  upsertError,
                  record
                );
              }
