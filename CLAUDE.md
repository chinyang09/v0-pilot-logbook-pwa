...
- Do not size the app shell with `100dvh` — pin `body` with `position: fixed; inset: 0` and size everything off `100%` of it. In an iOS standalone PWA `dvh` resolves to the SAFE viewport, so the app stopped short of the home indicator and left a dead band where content should have been flowing under it
- Do not derive the sidebar's open height from `window.innerHeight` — use `calc(100% - …)`, which resolves against the box a fixed element is actually laid out in. The measured version came out taller than the visible page on iPad Safari in portrait and overshot both ends. Subtract BOTH safe-area insets: one morph is top-anchored and the other bottom-anchored, and each still has to clear the other end
