...
- Do not inset the app shell by the safe area — the PWA runs edge to edge and content scrolls UNDER the status bar and Android's gesture pill. The insets belong inside each scroller and on the header's gradient, and `theme-color` must stay equal to the app background or the system bar reads as a band bolted on
- Do not make the floating header's gradient solid over the status bar — `.chrome-header-fade` anchors the solid part to the 64px action row and leaves the status-bar strip barely tinted, or content scrolling underneath is painted out and the strip reads as a blank pad (the whole point of edge to edge). Both headers share the class; change one, you change both
- Do not let `<meta name="theme-color">` follow only `prefers-color-scheme` — Android paints its system bars with it, and the app's theme is a class the user picks, so `ThemeColorMeta` keeps every theme-color meta on the RESOLVED app theme. iOS hides the problem (`black-translucent` leaves the strip transparent), Android does not
