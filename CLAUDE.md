...
- Do not size the shell with `100dvh`, and do not let ANY scroller chain its overscroll to the document. `body` is `height: 100%` — of `html`, i.e. the initial containing block — because `dvh` tracks the DYNAMIC viewport and the document's scrollable box is the ICB; when they disagreed the shell was taller than the document, and since body carries the `transform`, every `position: fixed` element (nav pill, headers, the mobile detail overlay) rode that scroll — the "free play" where action buttons scrolled off a page that had nothing to scroll. Every app scroller therefore carries `overscroll-contain` as well, so a flick that reaches the end of a list can never reach the page. Verified by sampling `scrollHeight − clientHeight` on the document continuously through load: 0 on every route
