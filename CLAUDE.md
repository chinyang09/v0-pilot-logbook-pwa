...
- Do not resize or un-transform the `body` shell. WebKit's installed-PWA bug reports the viewport (units AND the initial containing block) MINUS `safe-area-inset-bottom` under `viewport-fit=cover`, so `body` is the compensated application shell: `height: 100dvh` plus, **in standalone only**, `+ env(safe-area-inset-bottom)` (a browser tab's `100dvh` is already correct — compensating there re-creates the iPad-portrait overshoot). The `transform: translateZ(0)` on `body` is load-bearing: it makes body the containing block for every `position: fixed` element (nav pill, sidebar morph, drag lens, overlays), so they anchor to the compensated shell instead of the buggy viewport — remove it and the pill floats a full inset above the home indicator. Safe-area padding stays inside the bottom nav (`bottom: 4px + env(...)`), never on `body`
