...
- ✅ (requested) **Instant-feel pass — creates and primary tabs no longer route/mount:**
  - Mobile [+] on aircraft/airports/crew now opens the create form in the same `?selected` overlay cards use, via a transient `__new__` selection sentinel (never persisted to sessionStorage) + the existing `detailContent` — measured ~190ms tap-to-form vs a full route navigation. The `/new` routes remain for picker flows (choosing for a flight) and deep links. The create-guard is selection-aware: same selection or sentinel keeps the in-progress form; the user picking anything else ends create mode, so background syncs can't clobber the form and card taps can't show a stale one. Bonus: the iPad resize case needs no redirect at all now — the same state renders as overlay (mobile) or detail panel (desktop).
  - **Dashboard (`/`) and Roster joined the keep-alive set** — every primary tab now switches instantly with preserved scroll/state. Both pages gate their header actions with `usePageActive` and refresh their data on re-activation (accuracy preserved: SWR/Dexie hooks keep updating while hidden, plus an explicit refresh on return). Memory tradeoff is deliberate: the roster list is virtualized (bounded DOM) and the dashboard's FDP computation was already module-cached — the retained cost is mostly the Recharts trees.
  - Verified via Playwright: overlay opens ~190ms with `routed=false`, Back dismisses / [+] reopens, tab taps /→roster→logbook→dashboard leave exactly the expected retained hidden pages, and all earlier desktop/create/resize suites still pass.
- ✅ (requested) **Layout/breakpoint audit (mobile <720 / split 720–1119 / desktop-pill ≥1120), iPad resize focus:**
  - **Phantom 199px sidebar spacer at 920–1119px (real bug, reproduced+screenshotted):** `PushSidebar` reserved width from the persisted sidebar-open state at ≥920px, but the sidebar UI (the pill↔sidebar morph) only exists ≥1120px — an iPad-width window showed a dead left column pushing both panels right. Spacer (and the panel width-lock effect) now gate on the same ≥1120 desktop-pill breakpoint; the unused ≥920 hook was removed and the stale tier comment in `desktop-layout.tsx` corrected.
  - **Roster FastScroll rail floated over the detail panel on desktop:** it was `position:fixed` to the viewport (old style); now passed as `PageContainer rightContent` like every other list page, so it lives inside the main panel at all widths (verified geometrically).
  - **Shared breakpoint stores:** `use-is-desktop.ts` rewritten from per-consumer `useState`+4 listeners to three module-level stores consumed via `useSyncExternalStore` — every component now flips tiers in the same render pass (coherent layout switches, no per-component drift), listener count is constant, and the iPadOS resume-resync behavior is preserved verbatim.
  - Verified via a Playwright resize sweep (1300→1000→800→390→800→1300 and overlay handoffs): spacer 199px only at ≥1120, 0 elsewhere; exactly one nav variant at every width; open flight overlay at 390 hands off to the detail panel at 1300 and back to the overlay at 390; keep-alive pages survive the sweep.
