    ...
    useImperativeHandle(ref, () => ({
      scrollToFlight: (flightId: string, flightDate?: string) => {
        ...
        if (targetIndex >= 0 && targetIndex >= visibleCount) {
          setVisibleCount(
            Math.min(targetIndex + LOAD_INCREMENT, flights.length)
          );

          // Wait for render then scroll
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              const flightElement = document.getElementById(
                `flight-${flightId}`
              );
        ...
        if (flightElement) {
          ...
          if (monthFlightIndex >= 0) {
            const newVisibleCount = Math.min(
              monthFlightIndex + LOAD_INCREMENT,
              flights.length
            );
