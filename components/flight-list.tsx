            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between">
              <div className="flex flex-col">
                <div className="flex items-center justify-between gap-1">
                  <span className="text-base font-semibold leading-tight">{flight.outTime?.slice(0, 5) || ""}</span>
...
export const FlightList = forwardRef<FlightListRef, FlightListProps>(function FlightList(
  ...
  useImperativeHandle(ref, () => ({
    scrollToFlight: (flightId: string, flightDate?: string) => {
      const container = scrollContainerRef.current
      if (!container) return

      isExternalScrollRef.current = true
      lastDetectedFlightRef.current = flightId // Prevent bounce back

      // Find the index of the target flight in the full list
      const targetIndex = flights.findIndex((f) => f.id === flightId)

      // If flight exists in filtered list but not yet visible, load more
      if (targetIndex >= 0 && targetIndex >= visibleCount) {
        setVisibleCount(Math.min(targetIndex + LOAD_INCREMENT, flights.length))

        // Wait for render then scroll
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            const flightElement = document.getElementById(`flight-${flightId}`)
            if (flightElement && container) {
      ...
      // Flight is already loaded, scroll directly
      const flightElement = document.getElementById(`flight-${flightId}`)
      if (flightElement) {
        const containerRect = container.getBoundingClientRect()
        const elementRect = flightElement.getBoundingClientRect()
        ...
        // Flight not found by ID, try to find any flight in that month and load up to it
        const targetDate = parseDateLocal(flightDate)
        const targetYear = targetDate.getFullYear()
        const targetMonth = targetDate.getMonth()

        // Find first flight in target month
        const monthFlightIndex = flights.findIndex((f) => {
          const d = parseDateLocal(f.date)
          return d.getFullYear() === targetYear && d.getMonth() === targetMonth
        })

        if (monthFlightIndex >= 0) {
          const newVisibleCount = Math.min(monthFlightIndex + LOAD_INCREMENT, flights.length)
          setVisibleCount(newVisibleCount)

          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              const firstMonthFlight = flights[monthFlightIndex]
              const element = document.getElementById(`flight-${firstMonthFlight.id}`)
  ...
  }, [flights, visibleCount, onTopFlightChange, onScrollStart])
  ...
  return (
    <>
      <div ref={scrollContainerRef} className="h-full overflow-y-auto scroll-smooth" onScroll={onScroll}>
