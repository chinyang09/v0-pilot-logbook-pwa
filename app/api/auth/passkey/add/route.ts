...
export async function GET() {
  try {
    ...
    const options = generateRegistrationOptions(user._id, user.identity.callsign, user.auth.passkeys)
...
export async function POST(request: NextRequest) {
  try {
    ...
    await db.collection("sessions").updateOne({ _id: sessionId }, { $unset: { recoveryLogin: "" } })
