...
export async function POST(request: NextRequest) {
  try {
    const { callsign, code } = await request.json()

    if (!callsign || !code) {
      return NextResponse.json({ error: "Callsign and code required" }, { status: 400 })
    }

    const db = await getDB()

    // Find user by normalized callsign
    ...
    if (!user) {
      ...
      return NextResponse.json({ error: "Invalid callsign or code" }, { status: 401 })
    }
    ...
    await db.collection("sessions").insertOne({
      ...
      recoveryLogin: true, // Mark as recovery login
    })
