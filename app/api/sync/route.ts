...
export async function POST(request: NextRequest) {
  try {
    ...
    const result: { rejected?: boolean; reason?: string } = {};
    ...
    switch (type) {
      case "create":
      case "update":
        const isTombstoned = await checkTombstone(
          db,
          session.userId,
          collection,
          data.id
        ...
        if (isTombstoned) {
          console.log(
            `[v0] Rejecting ${type} for tombstoned record: ${data.id}`
          );
        ...
        if (type === "create") {
          ...
          if (existing) {
            ...
            if (incomingTime >= existingTime) {
              await coll.updateOne(
                { id: data.id, userId: session.userId },
                {
                  $set: {
                    ...dataWithUser,
          ...
          if (existingRecord) {
            ...
            const existingTime =
              existingRecord.updatedAt || existingRecord.createdAt || 0;
