...
import {
  LogbookCalendar,
  type CalendarHandle,
} from "@/components/logbook-calendar";
...
export default function LogbookPage() {
  ...
  const [activeFilterType, setActiveFilterType] = useState<
    "none" | "flight" | "aircraft" | "airport" | "crew"
  >("none");
  ...
  const lastScrollSourceRef = useRef<{
    source: "calendar" | "flights";
    timestamp: number;
  } | null>(null);
  ...
  const handleFlightScroll = useCallback(
    (topFlight: FlightLog | null) => {
      ...
      if (
        ...
        if (
          newYear !== selectedMonth.year ||
          newMonth !== selectedMonth.month
        ) {
          syncLockRef.current = true;
        ...
        switch (activeFilterType) {
          ...
          case "airport":
            return selectedFilters.some((filter) => {
              ...
              return (
                flight.departureIcao === icao || flight.arrivalIcao === icao
              );
            });
          case "crew":
            return selectedFilters.some((filter) => {
              if (flight.picName === filter || flight.sicName === filter)
                return true;
              if (flight.additionalCrew?.some((c) => c.name === filter))
                return true;
              return false;
            });
          default:
    ...
    <div className="min-h-screen bg-background flex flex-col overflow-hidden">
      ...
      >
        <div className="container mx-auto px-4 h-full relative">
          ...
          {searchFocused && (
            <div className="flex items-center gap-1.5 mt-2 p-0 bg-secondary/30 rounded-lg border border-border animate-in fade-in slide-in-from-top-5 duration-500">
              {[
                ...
                <Button
                  ...
                  className="flex-1 text-xs h-8 font-medium transition-all duration-500"
                >
                  {filter.label}
                </Button>
              ))}
      ...
      <main
        className="fixed left-0 right-0 bottom-12 flex flex-col overflow-hidden transition-all duration-500 ease-out"
        style={{
