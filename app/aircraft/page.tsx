...
export default function AircraftPage() {
        ...
        if (mounted) {
          ...
          for (const reg of recentRegs) {
            const found = aircraft.find(
              (ac) => ac.reg?.toUpperCase() === reg.toUpperCase()
            );
  ...
  return (
    <PageContainer
      header={
        <div className="flex-shrink-0 bg-card border-b border-border px-3 py-2 z-50">
      ...
      {isLoading ? (
        ...
        <div className="p-2 pb-safe space-y-2">
          ...
          {searchQuery.length >= 2 && (
            <>
              <h3 className="text-xs font-medium text-muted-foreground uppercase px-2">
